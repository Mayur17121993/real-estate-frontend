    ...
    <div className="container">
      <div className="table-header">
        ...
        <button className="property-button">
          <Link to={`/form/${email}`} className="link">
            + Add Property
          </Link>
        </button>
      </div>
        ...
        {data.map((data, index) => {
          return (
            <tr key={index} className="table-row">
              ...
              <td>{parseInt(Math.random() * 50)}</td>
              <td>
                <Link to={`/view/${data.ppdId}`}>
                  <button className="action">
                    <FiEye className="table-icons" />
                  </button>
                </Link>
                <button
                  className="action"
                  onClick={() => {
                    handleDelete(data.ppdId);
                  }}
                >
                  <FiDelete className="table-icons" />
                </button>
              </td>
            </tr>
          );
